  ...
  if (req.internalUrl && req.internalUrl.route && req.internalUrl.route.service) {
    ...
    uaaUtils.getUaaConfig(req, function (err, uaaOptions) {
      ...
      ((req.app.services[CONNECTIVITY][uaaOptions.tenant] &&
          ...
          !req.app.services[CONNECTIVITY][uaaOptions.tenant])) {
        getClientCredentialTokenByTenant(uaaOptions.tenant,connectivityCredentials,req.zoneInfo, function (err, token) {
          if (err) {
            return next(err);
          }
          req.app.services[CONNECTIVITY][uaaOptions.tenant] = {
...
function getClientCredentialTokenByTenant(tenant,credentials,zoneInfo, cb){
  return tokenUtils.getClientCredentialsTokenByTenant(tenant,credentials,zoneInfo, cb);
}
